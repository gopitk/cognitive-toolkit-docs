...
* Download the required CNTK custom MKL from the [CNTK web site](https://www.microsoft.com/en-us/cognitive-toolkit/download-math-kernel-library/).
  ...
  For example, if you are on latest master, download `CNTKCustomMKL-Linux-4.tgz` and extract its contents to `/usr/local/CNTKCustomMKL`, which should create a folder `/usr/local/CNTKCustomMKL/4` within:

```
sudo tar -xzf CNTKCustomMKL-Linux-4.tgz -C /usr/local/CNTKCustomMKL
```
